...
if (!fileId) {
  return NextResponse.json({ error: "Failed to get Drive file ID" }, { status: 500 });
}
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    if (!data?.driveId) return NextResponse.json({ error: "No Drive ID found for this file" }, { status: 400 });
    ...
    return NextResponse.json({ success: true, message: "File deleted from Drive and Firestore" });
  } catch (err) {
    console.error(err);
